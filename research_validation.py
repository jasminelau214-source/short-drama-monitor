...
def validate_research_payload(payload: object, *, allowed_source_urls: Iterable[str] | None = None) -> dict:
    """Deterministic gate for model research output.

    The model JSON schema is a generation aid, not a write authorization. This
    validator is intentionally run again before a task can become COMPLETE or
    write to drama_overrides.
    """
    errors: list[str] = []
    if not isinstance(payload, dict):
        return {'ok': False, 'errors': ['payload_not_object'], 'blankFields': list(CORE_FIELDS)}
