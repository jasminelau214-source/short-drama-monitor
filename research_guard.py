...
def _is_complete_ranking(run: dict) -> bool:
    ...
    for item in rows:
        ...
        try:
            ...
            return False
        if rank < 1 or rank > top_n or rank in ranks:
            return False
        ranks.add(rank)
...
def _run_titles(run: dict) -> set[str]:
    result = _json_obj(run.get('result_json'))
    rows = result.get('rows') if isinstance(result.get('rows'), list) else []
    return {normalize_title(x.get('title')) for x in rows if isinstance(x, dict) and normalize_title(x.get('title'))}


def assess_research_task(task: dict, analysis_runs: list[dict]) -> dict:
    """Check whether a task still belongs to the authoritative latest App run.

    A same-day later complete run supersedes an older run within the same
    platform/sourceType/targetKey scope. Dropped titles must not consume research
    APIs or contribute to the day's final analysis.
    """
    ...
    run_by_id = {str(r.get('id') or ''): r for r in analysis_runs}
    origin = run_by_id.get(task_run_id)
    if not origin:
        return {
            'current': False,
            'code': 'AUTHORITATIVE_RUN_UNRESOLVED',
            'reason': 'Task origin run is outside the available run window or missing.',
        }
    ...
    if source_type != 'SHORT_DRAMA_APP':
        return {
            ...
            'reason': f'Research auto-queue only accepts SHORT_DRAMA_APP, got {source_type}.',
        }
    ...
    if not candidates:
        return {
            ...
            'reason': 'No complete ranking run is available for the task scope.',
        }

    latest = max(candidates, key=lambda r: (str(r.get('updated_at') or ''), str(r.get('id') or '')))
    ...
    return {
        'current': True,
        'code': 'CURRENT',
        'reason': 'Task title is present in the latest authoritative run.',
        'latestRunId': latest_id,
        'targetKey': target_key,
